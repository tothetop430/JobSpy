...
class LinkedInScraper(Scraper):
    ...
    def _get_job_details(self, job_page_url: str) -> dict:
        ...
        try:
            ...
            response = session.get(
                job_page_url, headers=self.headers, timeout=500, proxies=self.proxy
            )
    ...
    def _parse_job_url_direct(self, soup: BeautifulSoup) -> str | None:
        ...
        #print("Hello there!\n")
        #print(f"{job_url_direct_content}")
        if job_url_direct_content:
            comment = job_url_direct_content.find(string=lambda text: isinstance(text, Comment))
            job_url_direct=comment.string.replace("\"", "")
        else:
            job_url_direct="None"
